...
> ```
> dnx generatorlog@0.0.5-alpha --verbose -- dotnet build
> ```
...
```bash
dnx generatorlog@0.0.5-alpha --output ~/traces/mybuild.nettrace -- dotnet build

# Windows (ETW)
dnx generatorlog@0.0.5-alpha --output C:\traces\mybuild.etl
```

On Windows, trace a specific process via EventPipe instead of system-wide ETW:

```
dnx generatorlog@0.0.5-alpha --pid 12345
```

For full help:

```
dnx generatorlog@0.0.5-alpha --help
```
